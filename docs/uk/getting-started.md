...
Для локальної розробки без повної контейнеризації додатково потрібні Flutter `3.41.9`, .NET SDK 9 і Chrome або інший Flutter web target. PostgreSQL локально встановлювати не обов’язково: базу можна запустити окремим Docker-сервісом.
...
`.env` ігнорується Git. Не додавай у нього production credentials. Початкові значення підходять для локального старту, але файлові операції S3 вимагатимуть реальних тестових AWS credentials і bucket.
...
Під час першого старту API автоматично застосовує EF Core migrations. Якщо `ADMIN_EMAIL` і `ADMIN_PASSWORD` задані, створюється локальний admin, якщо користувача з таким email ще немає.
...
- `api` не завершується з configuration exception;

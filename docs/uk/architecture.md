...
`frontend/lib/features/auth` розділяє HTTP-репозиторій, сховище сесії, контролер і сторінки. Реєстрація викликає `POST /api/Auth/register`, а потім виконує вхід через `POST /api/Auth/login`. Профіль заповнюється з identity claims JWT. Access і refresh token зберігаються через `shared_preferences`; пароль не зберігається. Обрана мова та світла/темна тема використовують ту саму абстракцію сховища й відновлюються під час запуску.
...
- Вхід, реєстрація, локальне відновлення сесії, перегляд профілю та вихід мають завершені frontend-сценарії. Автоматичне оновлення token-ів, віддалене редагування профілю, відновлення пароля та завантаження аватара ще не підключені.
- Надсилання відгуків поки змінює лише локальний стан frontend, а файлові операції ще не мають завершеного UI.

...
Звичайний запуск використовує лише `api`. Режим `test` також звертається до API та БД, але додає до отриманих рецептів локальний демо-каталог. Якщо каталог API недоступний, залишаються доступними mock-рецепти. Авторизація в обох режимах справжня й вимагає доступного API.
